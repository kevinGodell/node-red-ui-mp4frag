...
module.exports = ($scope, events) => {
  ...
  const shouldLoad = () => {
    ...
    return documentVisible === true && /* isAncestorVisible() === true &&*/ ((documentFullscreen === false && videoIntersecting === true) || (documentFullscreen === true && videoFullscreen === true));
  };
  ...
  $scope.init = config => {
    ...
    /* isAncestorVisible = (() => {
      const ancestor = videoElement.closest('div.nr-dashboard-cardcontainer');
