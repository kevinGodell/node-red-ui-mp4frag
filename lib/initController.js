...
module.exports = ($scope, events) => {
  ...
  const extractVideoData = payload => {
    ...
    if (typeof payload === 'string') {
      ...
      const { hlsPlaylist, socketServer, mp4File } = payload;

      if (typeof hlsPlaylist === 'string' && hlsPlaylist.endsWith('.m3u8') === true) {
        type = 'hls';

        source = hlsPlaylist;
      } else if (typeof socketServer === 'string' && socketServer.endsWith('socket.io') === true) {
        type = 'socket.io';
  ...
  const initVideo = (type, source) => {
    ...
    switch (type) {
      ...
      case 'socket.io':
        ...
        $scope.socket = io({ path: source, transports: ['websocket', 'polling'] });
