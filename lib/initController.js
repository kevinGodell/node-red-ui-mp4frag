  ...
  const extractVideoData = payload => {
    ...
    for (let i = 0; i < $scope.config.players.length; ++i) {
      switch ($scope.config.players[i]) {
        ...
        case 'hls.js':
          if (hasHlsJs() === true) {
            const source = typeof payload === 'string' ? payload : typeof hlsPlaylist === 'string' ? hlsPlaylist : null;
        ...
        case 'mp4':
          const source = typeof payload === 'string' ? payload : typeof mp4Video === 'string' ? mp4Video : null;
          ...
          const load = () => {
            ...
            socketIo.on('mime', data => {
              ...
              videoElement.onerror = err => {
                log('error', `videoElement.onerror() - ${err.name} - ${err.message}`);
              ...
              videoElement.onloadedmetadata = async event => {
                ...
                try {
                  if ($scope.config.autoplay !== false) {
              ...
              mediaSource.onsourceopen = () => {
                ...
                sourceBuffer.onerror = err => {
                  log('error', `sourceBuffer.onerror() - ${err.name} - ${err.message}`);
